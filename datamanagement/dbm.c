...
int mian()
{
	strcut test_data items_to_stores[ITEMS_USED];
	...
	key_datim.dsize = strlen(key_to_use);

	data_datum = dbm_fetch(dbm_ptr, key_datum);
	if(data_datum.dptr)
	{
		printf("Data retrieved\n");
		memcpy(&item_retrieved, data_datum.ptr, data_datum.dsize);
		printf("Retrieved item -%s %d %s\n"
			   item_retrieved.misc_chars,
			   item_retrieved.integer,
