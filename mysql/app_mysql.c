...
int add_cd(char *artist, char *title, char *catalogue, int *cd_id)
{
	...
	if(res)
	{
		fprintf(stderr, "Inser error %d: %s\n", mysql_errno(&my_connection), mysql_error(&my_connection));
		return 0;
	}
...
int get_cd(int cd_id, struct current_cd_st *dest)
{
	...
	sprintf(qs, "SELECT artist_id, cd.id, artist.name, cd.title, cd.catalogue FROM artist, cd WHERE artist.id = cd.artist_id and cd.id = %d", cd_id);
	res = mysql_query(&my_connection, qs);
	if(res)
	{
		fprintf(stderr, "SELECT error: %s\n", mysql_error(&my_connection));
